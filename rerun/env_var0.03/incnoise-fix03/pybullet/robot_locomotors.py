...
class WalkerBase(MJCFBasedRobot):
  ...
  def __init__(self, fn, robot_name, action_dim, obs_dim, power):
    ...
    self.body_xyz = [0, 0, 0]
    self.posturerange = 0.03

  def setposturerange(self, nrange):
    self.posturerange = nrange

  def robot_specific_reset(self, bullet_client):
    self._p = bullet_client
    for j in self.ordered_joints:
      j.reset_current_position(self.np_random.uniform(low=-self.posturerange, high=self.posturerange), 0)
