...
class Algo(EvoAlgo):
    ...
    def setProcess(self):
        ...
        self.current_fitness_calc_mode = 0
        self.fitness_calc_mode_weights = [(0,1,0),(0.33333,0,0.66666),(0.5,0,0.5),(0.66666,0,0.33333)]
    ...
    def change_fitness_calc_mode(self,mode:int):
        print(f"Changing the way of calculating the fitness - Mode = {mode} - The new weights for max, avg, min are {self.fitness_calc_mode_weights[mode]}")
        self.policy.max_fitness_weight = self.fitness_calc_mode_weights[mode][0]
        self.policy.min_fitness_weight = self.fitness_calc_mode_weights[mode][1]
        self.policy.avg_fitness_weight = self.fitness_calc_mode_weights[mode][2]
        self.current_fitness_calc_mode = mode
    
    def next_fitness_calc_mode(self):
        self.change_fitness_calc_mode((self.current_fitness_calc_mode + 1)%len(self.fitness_calc_mode_weights))
    ...
    def run(self):
        ...
        #size of the moving average for defining stagnation
        gen_window = 50
        #threshold of improvement used for defining stagnation
        improv_threshold = 0.02
        #moving average records
        mov_avg_values = []
        #creating a CircBuffer to facilitate the moving average calculation
        #defining it here because it supposed to be local
        class CircBuffer:
        ...
        last_gen_fitness = CircBuffer(gen_window)
        fitMode = []
        ...
        while (self.steps < self.maxsteps):
            ...
            last_gen_fitness.add(self.bfit)
            fitMode.append(self.current_fitness_calc_mode)            
            if self.cgen!=0:
                if self.cgen%gen_window==0:                    
                    ...
                    if self.cgen>=2*gen_window and not fitModeChanged:
                        improv_rate = (mov_avg_values[-1]/mov_avg_values[-2])-1
                        ...
                        if improv_rate < improv_threshold:                            
                            print("PROCESS STAGNATED - CHANGING FITNESS CALCULATION MODE")
                            self.next_fitness_calc_mode()
                            fitModeChanged = True
                        else:
                            print("PROCESS IMPROVING - FITNESS CALCULATION MODE NOT CHANGED")
                    else:
                        print("FITNESS CALCULATION MODE NOT CHANGED - NOT ENOUGH DATE FOR CURRENT CALCULATION MODE (2 windows)")
                        fitModeChanged = False
        ...
        np.save(self.filedir + "/S" + str(self.seed) + ".fitMode",fitMode)
        self.ievReg = []
