...
class BulletPolicy(Policy):
    ...
    def rollout(self, ntrials, render=False, seed=None):   # evaluate the policy for one or more episodes 
        ...
        self.nn.setnoiserange(0.0)      # noise is generated here below, so we set the noiserange generated by evonet to 0.0         
        if (self.test == 2):
            ...
            while t < self.maxsteps:
                ...
                if (ntrials == 1):
                    if (t > 0 and (t % 100) == 0):
                        ...
                        nrange += 0.02
                if ((t % nsteps) == 0):
                    motornoise = np.random.randn(self.noutputs) * nrange
                self.ob, r, done, _ = self.env.step(self.ac+motornoise)
                #print(motornoise)
                rew += r
                t += 1
                if (self.test > 0):
